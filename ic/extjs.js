...
import helloIDL from "./candid/loka.did.js";
...
for (const a in tokensToLoad) {
  if (tokensToLoad.hasOwnProperty(a)) {
    loadedTokens[a] = [];
    for (
      var i = tokensToLoad[a][0];
      i < tokensToLoad[a][1] + tokensToLoad[a][0];
      i++
    ) {
...
class ExtConnection {
  ...
  _mapIdls = {
    ...
    "bkyz2-fmaaa-aaaaa-qaaaq-cai": helloIDL,
  };
  _metadata = {
    [LEDGER_CANISTER_ID]: {
      name: "ICP",
  ...
  constructor(host, identity) {
    ...
    this._host = "bkyz2-fmaaa-aaaaa-qaaaq-cai";
    this._makeAgent();
  }
  idl(canister, idl) {
...
const extjs = {
  connect: (host, identity) =>
    new ExtConnection(host ?? "https://icp0.io/", identity),
